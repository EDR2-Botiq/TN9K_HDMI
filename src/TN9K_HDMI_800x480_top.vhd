...
architecture rtl of TN9K_HDMI_800x480_top is
    ...
    signal tmds_clock_internal : std_logic;
    ...
    -- HDMI Core component declaration is now in hdmi_package
...
begin
    ...
    -- Video active and pixel coordinate generation (cx/cy come from hdmi module)
    ...
    hdmi_core_inst : hdmi
        generic map (
            VIDEO_ID_CODE => 200,  -- Custom 800x480 mode
            IT_CONTENT => '1',
            BIT_WIDTH => 11,
            BIT_HEIGHT => 10,
            DVI_OUTPUT => '0',
            ...
            PRODUCT_DESCRIPTION => x"465047412D44656D6F20202020202000",  -- "FPGA-Demo    \0" (16 bytes)
            SOURCE_DEVICE_INFORMATION => x"00",
            START_X => 0,
            START_Y => 0
        )
        port map (
            ...
            reset => reset,
            rgb => rgb_data,
            audio_sample_word_0 => audio_left,   -- Left channel
            audio_sample_word_1 => audio_right,  -- Right channel
            tmds => tmds_p_internal,
    ...
    -- Output assignments (need to generate differential signals)
    O_tmds_data_p <= tmds_p_internal;
    O_tmds_data_n <= not tmds_p_internal;
    O_tmds_clk_p <= tmds_clock_internal;
    O_tmds_clk_n <= not tmds_clock_internal;
